      ...
      @foreach ($posts->skip(1) as $post)
          <div class="group relative cursor-pointer items-center justify-center overflow-hidden transition-shadow hover:shadow-xl hover:shadow-black/30 rounded-lg">
            ...
            <div class="absolute inset-0 flex translate-y-[60%] flex-col items-center justify-center px-9 transition-all duration-500 group-hover:translate-y-0">
              <h1 class="mb-4 font-dmserif text-xl font-bold text-white">{{ $post->title }}</h1>
              <p class="mb-3 text-sm italic text-white opacity-0 transition-opacity duration-300 group-hover:opacity-100 ">{{ $post->excerpt }}</p>
              <a href="/posts/{{ $post->slug }}" class="rounded-full bg-neutral-900 py-2 px-3.5 font-com text-sm capitalize text-white shadow shadow-black/60 hover:bg-biru">See More</a>
            </div>
          </div>
